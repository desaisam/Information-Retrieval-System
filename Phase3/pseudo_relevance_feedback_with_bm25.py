...
OUTPUT_FILE = 'pseudo_relevance_feedback_with_bm25.txt'
...
def bm25_search(query, max_results):
  qtf = queryTermFrequency(query)
  indexToUse = query_matching_index(qtf, inverted_idx)
  
  results = BM25_score(indexToUse, queryTermFrequency(query), tokenCountInDocument, "../preprocessing/cacm.rel.txt")
...
def search(query):
  original_search = bm25_search(query, k)
  ...
  return bm25_search(expanded_query, MAX_RESULTS)
